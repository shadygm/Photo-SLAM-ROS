...
GaussianMapper::GaussianMapper(std::shared_ptr<ORB_SLAM3::System> pSLAM,
                               ...
                               std::filesystem::path result_dir, int seed,
                               torch::DeviceType device_type)
    : pSLAM_(pSLAM), initial_mapped_(false), interrupt_training_(false),
      stopped_(false), iteration_(0), ema_loss_for_log_(0.0f),
      SLAM_ended_(false), loop_closure_iteration_(false),
      min_num_initial_map_kfs_(15UL), large_rot_th_(1e-1f),
      large_trans_th_(1e-2f), training_report_interval_(0) {
  // Random seed
  std::srand(seed);
...
void GaussianMapper::run() {
  ...
  while (!isStopped()) {
    ...
    // TODO: Here implement the ROS node for publishing the gaussian map
  }
...
void GaussianMapper::trainForOneIteration() {
  ...
  if (pSLAM_) {
    ...
    int step = (used_times <= opt_params_.position_lr_max_steps_
                    ? used_times
                    : opt_params_.position_lr_max_steps_);
...

  {
    ...
    if (keyframe_record_interval_ &&
      ...
      recordKeyframeRendered(masked_image, gt_image, viewpoint_cam->fid_,
    ...
    if (getIteration() < opt_params_.densify_until_iter_) {
      ...
      // if (!isdoingGausPyramidTraining() || training_level <
      // num_gaus_pyramid_sub_levels_)
      ...
      if ((getIteration() > opt_params_.densify_from_iter_) &&
        ...
        int size_threshold =
            (getIteration() > prune_big_point_after_iter_) ? 20 : 0;
    ...
    if (training_report_interval_ &&
      ...
      GaussianTrainer::trainingReport(
          getIteration(), opt_params_.iterations_, Ll1, loss, ema_loss_for_log_,
          loss_utils::l1_loss, iter_time, *gaussians_, *scene_, pipe_params_,
          background_);
    ...
    case ORB_SLAM3::MappingOperation::OprType::LoopClosingBA: {
      ...
      for (auto &kf : associated_kfs) {
        ...
        if (pkf) {
          ...
          // if (std::get<4>(kf)) {
          // renderAndRecordKeyframe(pkf, result_dir_,
          ...
          if (large_rot || large_trans) {
            ...
            diff_pose.translation() -=
                inv_pose.translation(); // t = (R_new * t_old + t_new) - t_new
            ...
            increaseKeyframeTimesOfUse(pkf,
                                       loop_closure_increased_times_of_use_);
            // renderAndRecordKeyframe(pkf, result_dir_,
            // "_1_after_loop_transforming_points");
          ...
          pkf->setPose(pose.unit_quaternion().cast<double>(),
                       pose.translation().cast<double>());
          pkf->computeTransformTensors();
          // if (std::get<4>(kf)) renderAndRecordKeyframe(pkf, result_dir_,
          // "_2_after_pose_correction");
        } else {
          handleNewKeyframe(kf);
        }
      }
      ...
      // keyframesToJson(result_dir_ / (std::to_string(getIteration()) +
      // "_0_before_loop_correction"));
...
void GaussianMapper::handleNewKeyframe(
    std::tuple<unsigned long /*Id*/, unsigned long /*CameraId*/,
               Sophus::SE3f /*pose*/, cv::Mat /*image*/, bool /*isLoopClosure*/,
               cv::Mat /*auxiliaryImage*/, std::vector<float>,
               std::vector<float>, std::string> &kf) {
...
void GaussianMapper::increaseKeyframeTimesOfUse(
    std::shared_ptr<GaussianKeyframe> pkf, int times) {
...
void GaussianMapper::increasePcdByKeyframeInactiveGeoDensify(
  ...
  case MONOCULAR: {
    ...
    auto result =
        monocularPinholeInactiveGeoDensifyBySearchingNeighborhoodKeypoints(
            ...
            monocular_inactive_geo_densify_max_pixel_dist_, pkf->intr_,
            pkf->image_width_);
  ...
  case STEREO: {
    ...
    point_valid_flags = torch::logical_and(
        point_valid_flags,
        torch::where(disp >
                         static_cast<float>(stereo_cv_sgm_->getMinDisparity()),
                     true, false));
    point_valid_flags = torch::logical_and(
        point_valid_flags,
        torch::where(
  ...
  case RGBD: {
    ...
    point_valid_flags = torch::logical_and(
        point_valid_flags, torch::where(depth > RGBD_min_depth_, true, false));
    point_valid_flags = torch::logical_and(
        point_valid_flags, torch::where(depth < RGBD_max_depth_, true, false));
...
void GaussianMapper::recordKeyframeRendered(
    torch::Tensor &rendered, torch::Tensor &ground_truth, unsigned long kfid,
    std::filesystem::path result_img_dir, std::filesystem::path result_gt_dir,
    std::filesystem::path result_loss_dir, std::string name_suffix) {
  ...
  if (record_ground_truth_image_) {
    ...
    cv::imwrite(result_gt_dir /
                    (std::to_string(getIteration()) + "_" +
...
cv::Mat GaussianMapper::renderFromPose(const Sophus::SE3f &Tcw, const int width,
                                       const int height,
  ...
  if (main_vision)
    masked_image =
        std::get<0>(render_pkg) * viewer_main_undistort_mask_[pkf->camera_id_];
  else
    masked_image =
        std::get<0>(render_pkg) * viewer_sub_undistort_mask_[pkf->camera_id_];
  return tensor_utils::torchTensor2CvMat_Float32(masked_image);
}

void GaussianMapper::renderAndRecordKeyframe(
    std::shared_ptr<GaussianKeyframe> pkf, float &dssim, float &psnr,
    float &psnr_gs, double &render_time, std::filesystem::path result_img_dir,
    std::filesystem::path result_gt_dir, std::filesystem::path result_loss_dir,
  ...
  auto rendered_image = std::get<0>(render_pkg);
  torch::Tensor masked_image =
      rendered_image * undistort_mask_[pkf->camera_id_];
  torch::cuda::synchronize();
  auto end_timing = std::chrono::steady_clock::now();
  ...
  dssim =
      loss_utils::ssim(masked_image, gt_image, device_type_).item().toFloat();
  psnr = loss_utils::psnr(masked_image, gt_image).item().toFloat();
  psnr_gs = loss_utils::psnr_gaussian_splatting(masked_image, gt_image)
                .item()
                .toFloat();

  recordKeyframeRendered(masked_image, gt_image, pkf->fid_, result_img_dir,
